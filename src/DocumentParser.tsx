...
const DocumentParser: React.FC = () => {
    ...
    //     region:'us-east-1'
    //   });
    const textract = new Textract({ region: 'us-east-1' , credentials:{
        accessKeyId: 'ASIAXBDE6P26L5I7LZFR',
        secretAccessKey: 'T3jjnNascyHD79hh5hn129OfORLUvGyVCZBFUx5X',
        sessionToken: '',
    }});
  ...
  const parseS3Document = async(s3ObjectKey: string) => {
    const startTextDetectionParams = {
        DocumentLocation: {
          S3Object: {
            Bucket: 'gconbucket',
            Name: s3ObjectKey,
    ...
    textract.startDocumentTextDetection(startTextDetectionParams, (err, data) => {
        if (err) {
          console.log('Error starting text detection:', err);
        } else {
          console.log('Text detection started successfully:', data);
        }
      });
  ...
  const uploadDocumentToS3 = async (documentBytes: Uint8Array, s3ObjectKey: string)=> {
  ...
  const parseDocument = async () => {
    if (file) {
      ...
      fileReader.onload = async () => {
        ...
        uploadDocumentToS3(documentBytes,s3ObjectKey);
